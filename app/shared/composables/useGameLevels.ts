/**
 * useGameLevels — driver for /student/game level-path mini-games.
 *
 *  - `levels`           — static mock catalogue (sorted by order)
 *  - `progress`         — async-loaded per-level status from GameAttempt (best, attempts, passed)
 *  - `isUnlocked(id)`   — level N is unlocked when level N-1 is passed (or N === 1)
 *  - `sampleQuestions`  — Fisher-Yates random subset of size `questionsPerAttempt`
 *  - `recordAttempt`    — inserts GameAttempt + XpLog if passed
 *
 * Anti-abuse: pool ≥ 2 × sample, fresh shuffle per attempt, and failed attempts
 * still consume an entry but never reveal the right answer in feedback.
 */
...
export const useGameLevels = () => {
  ...
  /** Level is unlocked if previous one is passed (or it's the first level). */
  ...
  /** Convenience: derived list with status flags for the path UI. */
  ...
  const recordAttempt = async (
    ...
    let xpAwarded = 0
    if (passed) {
      // Only award full XP on the FIRST successful pass; subsequent passes earn nothing.
