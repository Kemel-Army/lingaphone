/**
 * Lingafon-student data composable — drop-in replacement for `useMockStudent()`.
 * Loads everything for the current student in parallel and exposes reactive refs.
 *
 * NOTE on shape: returns Vue refs/computeds, not plain objects.
 * In templates Vue auto-unwraps. In <script setup>, use `.value`.
 */

import type { Database, Json } from '~/shared/types/database.types'
import {
  PRACTICE_DECKS, MOCK_WORD_OF_DAY
} from '~/shared/mock'
...
/** Group enriched with teacher (flattened) and branch */
...
export const useLingafonStudent = () => {
  ...
  const { data, refresh, pending } = useAsyncData(
    ...
    async () => {
      ...
      // 1. Resolve student profile by authId
      ...
      // Need GroupMember counts for each visible group — single query
      ...
      // Real classmates: for each visible group fetch all members + user info
      ...
      // Build the enriched groups with flattened teacher
      ...
      // Daily streak — consecutive days with any PracticeAttempt OR XpLog entry,
      ...
      for (let i = 0; i < 365; i++) {
        ...
        if (activityDays.has(key)) {
          ...
          // Allow today to be missing if streak > 0 (count from yesterday)
          if (i === 0) {
            cursor.setDate(cursor.getDate() - 1)
            continue
          }
      ...
      // Today practice count — used to drive daily quest progress
  ...
  // ─── Derived refs (computed for templates) ─────────────────────────────
  ...
  const profile = computed(() => {
    ...
    return {
      ...
      // Prefer computed values over denormalized Student row
      dailyStreak: data.value?.dailyStreak ?? s.dailyStreak,
      totalXp: data.value?.totalXp ?? s.totalXp
    }
  })

  /** XP level table — linear 100 XP per level for simplicity */
  ...
  /** Real classmates for a given groupId — excludes current student */
  ...
  /** Today's practice count — drives daily quest progress */
  const practiceCountToday = computed(() => data.value?.practiceCountToday ?? 0)

  /** Live, data-driven daily quests. Pure computed — no DB writes needed. */
  ...
  /** Grades enriched with topic, group, teacher — for the journal page */
  ...
  /**
   * Submit homework + award XP for on-time submission.
   *   +50 XP if submitted before dueAt
   *   +25 XP if submitted after dueAt
   * XP is logged only once per (homework, student) — re-submit gives no extra XP.
   */
  const submitHomework = async (homeworkId: string, answers: Json, aiScore?: number) => {
    if (!data.value) return null
    const studentId = data.value.studentId
    const hw = data.value.homeworks.find(h => h.id === homeworkId)
    const previouslySubmitted = data.value.submissions.some(s =>
  ...
  /**
   * Record a practice card attempt + grant card-level XP.
   *   ≥85% → +5 XP
   *   60-84% → +3 XP
   *   <60% → 0 XP (still recorded as attempt)
   */
  const recordPracticeAttempt = async (
    ...
    await supabase.from('PracticeAttempt').insert({
      studentId, deckId, cardId, target, transcript, score
    })
    const amount = score >= 85 ? 5 : score >= 60 ? 3 : 0
    if (amount > 0) {
      await supabase.from('XpLog').insert({
        studentId, action: 'PRACTICE_CARD', amount, refId: cardId
      })
    }
  }

  /**
   * Record a finished deck and grant deck-completion XP.
   *   ≥85% avg → +50 XP
   *   ≥60% avg → +25 XP
   *   <60% avg → 0 XP
   * Idempotent within a day — same deck can be replayed but XP is awarded once per day.
   */
  const recordDeckCompletion = async (deckId: string, avgScore: number) => {
    ...
    if (amount === 0) return
    // Has the user already earned deck XP for this deck today?
    ...
    await supabase.from('XpLog').insert({
      studentId, action: 'PRACTICE_DECK', amount, refId: deckId
    })
    await refresh()
  }

  /**
   * Save a word to personal vocabulary and grant XP only when it's a new entry.
   */
  const addToVocabulary = async (
    ...
    if (!data.value) return
    const studentId = data.value.studentId
    const alreadySaved = data.value.vocabulary.some(v => v.word.toLowerCase() === word.toLowerCase())

    await supabase.from('VocabularyEntry').upsert({
      studentId, word, ipa, translation, example
    }, { onConflict: 'studentId,word' })

    if (!alreadySaved) {
      await supabase.from('XpLog').insert({
        studentId, action: 'MANUAL_AWARD', amount: 10, refId: `vocab:${word}`
      })
    }
    await refresh()
  }

  // Word of the day — rotate through a small seed list by day-of-year
  ...
  return {
    ...
    wordOfDay,
    classmatesOf,
    // Content (no per-student DB table yet — these are templates, not user data)
    practiceDecks: PRACTICE_DECKS,
    // Pure-empty until Certificate / Event tables are migrated
