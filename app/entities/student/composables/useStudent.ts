...
// normalizeSchedule/ScheduleSlot — auto-imported from ~/shared/lib/schedule
  ...
  const { data, refresh, pending } = useAsyncData(
    ...
    async () => {
      ...
      const groups: StudentGroup[] = ((groupsRes.data ?? []) as Array<{ group: GroupResolved | null }>)
        ...
        .map(g => ({
          ...g,
          schedule: normalizeSchedule((g as { schedule?: unknown }).schedule),
          teacher: g.teacher
            ? {
                id: g.teacher.id,
